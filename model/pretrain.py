...
import pickle
train_val_prev_trajs=json.load(open('./datasets/nuscene_gpvl_v2/captions_nuscene_train_val_prevtrajs.json'))
with open('./nuscene_infos/train_val_cmds.pickle', 'rb') as file:
   train_val_cmds = pickle.load(file)
...
class VALOR(VALORModel):
    ...
    def __init__(self, opts):
        super().__init__(opts)
        config = opts
        self.train_val_cmds = train_val_cmds
        self.train_val_prev_trajs = train_val_prev_trajs
        ...
        #self.fineweight_type = 'none'  ## direct train captioning
        ...
        if self.txt_encoder_type.startswith('clip') and self.video_encoder_type.startswith('clip') and opts.init_clip_head:      
            ...
            self.contra_head_t_det = Contra_head(self.txt_dim, contra_dim)
            self.contra_head_t_motion = Contra_head(self.txt_dim, contra_dim)
            self.contra_head_t_map = Contra_head(self.txt_dim, contra_dim)
            self.contra_head_v = Contra_head(self.video_dim, contra_dim)
            self.contra_head_v_det = Contra_head(self.video_dim, contra_dim)
            self.contra_head_v_motion = Contra_head(self.video_dim, contra_dim)
            self.contra_head_v_map = Contra_head(self.video_dim, contra_dim)
            
        self.det_motion_map_linear = nn.Sequential(nn.Linear(self.txt_dim*3, self.txt_dim), nn.ReLU(inplace=True))
        ...
        if self.contra_type == 'fine':
            ...
            self.audio_fine_weight = nn.Sequential(
                    nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),
                    nn.Linear(contra_dim, 1))
            self.fine_weight_mapper = {'text':self.text_fine_weight,'video':self.video_fine_weight,'audio':self.audio_fine_weight}
            ## det-motion-map
            self.text_fine_weight_det = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.video_fine_weight_det = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.text_fine_weight_motion = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.video_fine_weight_motion = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.text_fine_weight_map = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.video_fine_weight_map = nn.Sequential(nn.Linear(contra_dim, contra_dim), nn.ReLU(inplace=True),nn.Linear(contra_dim, 1))
            self.fine_weight_mapper_det = {'text':self.text_fine_weight_det,'video':self.video_fine_weight_det}
            self.fine_weight_mapper_motion = {'text':self.text_fine_weight_motion,'video':self.video_fine_weight_motion}
            self.fine_weight_mapper_map = {'text':self.text_fine_weight_map,'video':self.video_fine_weight_map}
    ...
    def forward_pt(self, batch, task, compute_loss=True):
        ...
        txt_tokens_det = batch['txt_tokens_det']
        txt_tokens_motion = batch['txt_tokens_motion']
        txt_tokens_map = batch['txt_tokens_map']
        txt_tokens = batch['txt_tokens_cap']
        video_pixels = batch['video_pixels']
        audio_spectrograms = batch['audio_spectrograms']       
        ids = batch['ids']
        commands = []
        # for name in ids:
        #     cmd = self.train_val_cmds[name]
        #     if cmd[0]==1:
        #        command = 'turn right'
        #     elif cmd[1]==1:
        #         command = 'turn left'
        ...
        for name in ids:
            prev_trajs = self.train_val_prev_trajs[name][0]
            commands.append(prev_trajs)

        video_output = None
        audio_output = None
        txt_tokens_contra_det = None
        txt_tokens_contra_motion = None  
        txt_tokens_contra_map = None 
        ...
        if 't' in ''.join(contra_task):
            txt_tokens_contra_det = self.get_text_tokens(txt_tokens_det, self.txt_encoder_type)
            txt_tokens_contra_motion = self.get_text_tokens(txt_tokens_motion, self.txt_encoder_type)
            txt_tokens_contra_map = self.get_text_tokens(txt_tokens_map, self.txt_encoder_type)
            txt_tokens_contra=torch.cat((txt_tokens_contra_det, txt_tokens_contra_motion, txt_tokens_contra_map), dim=1)
            batch_size = txt_tokens_contra_det.shape[0]
            self.use_task_prompt=False
            if  self.use_task_prompt:
                #task_prompt = self.get_task_prompt('project language in common space',batch_size)
                task_prompt = self.get_task_prompt_ltp('describe the video with natural language', commands, batch_size)
            else:
                task_prompt = None 
            
            txt_tokens_len = txt_tokens_contra_det.shape[1]
            txt_output_det = self.forward_txt_encoder(txt_tokens_contra_det, task_prompt)
            txt_output_motion = self.forward_txt_encoder(txt_tokens_contra_motion, task_prompt)
            txt_output_map = self.forward_txt_encoder(txt_tokens_contra_map, task_prompt)
            if  self.use_task_prompt:
                txt_output_det = txt_output_det[:,:txt_tokens_len]
                txt_output_motion = txt_output_motion[:,:txt_tokens_len]
                txt_output_map = txt_output_map[:,:txt_tokens_len]
            
        video_output = self.mlp(video_output)
        if contra_task != []:
            ...
            if 't' in ''.join(contra_task):
                txt_pooled_det = self.pool_text_for_contra(txt_output_det, txt_tokens_contra_det)
                txt_pooled_motion = self.pool_text_for_contra(txt_output_motion, txt_tokens_contra_motion)
                txt_pooled_map = self.pool_text_for_contra(txt_output_map, txt_tokens_contra_map)
                txt_pooled = torch.cat((txt_pooled_det, txt_pooled_motion, txt_pooled_map), dim=1)
                ## global
                feat_t = self.contra_head_t(txt_pooled)
                feat_t = F.normalize(feat_t,dim=-1)
                ## det
                feat_t_det = self.contra_head_t_det(txt_pooled_det)
                feat_t_det = F.normalize(feat_t_det,dim=-1)
                ## motion
                feat_t_motion = self.contra_head_t_motion(txt_pooled_motion)
                feat_t_motion = F.normalize(feat_t_motion,dim=-1)
                ## map
                feat_t_map = self.contra_head_t_map(txt_pooled_map)
                feat_t_map = F.normalize(feat_t_map,dim=-1)
                if compute_loss:
                    feat_t = ddp_allgather_with_grads.apply(feat_t)
                    txt_tokens_contra = ddp_allgather(txt_tokens_contra)
                    ## det-motion-map
                    feat_t_det = ddp_allgather_with_grads.apply(feat_t_det)
                    txt_tokens_contra_det = ddp_allgather(txt_tokens_contra_det)
                    feat_t_motion = ddp_allgather_with_grads.apply(feat_t_motion)
                    txt_tokens_contra_motion = ddp_allgather(txt_tokens_contra_motion)
                    feat_t_map = ddp_allgather_with_grads.apply(feat_t_map)
                    txt_tokens_contra_map= ddp_allgather(txt_tokens_contra_map)
            if 'v' in ''.join(contra_task):
                #video_pooled = self.pool_video_for_contra(video_output)
                ## global
                video_pooled = torch.mean(video_output, dim=2) ## video_output (B,1,700,768)  video_pooled (B, 1, 768)
                feat_v = self.contra_head_v(video_pooled)
                feat_v = F.normalize(feat_v,dim=-1)
                ## det
                video_pooled_det= torch.mean(video_output[:,:,:300,:], dim=2) 
                feat_v_det = self.contra_head_v_det(video_pooled_det)
                feat_v_det = F.normalize(feat_v_det,dim=-1)
                ## motion
                video_pooled_motion = torch.mean(video_output[:,:,300:600,:], dim=2) 
                feat_v_motion = self.contra_head_v_motion(video_pooled_motion)
                feat_v_motion = F.normalize(feat_v_motion,dim=-1)
                ## map
                video_pooled_map = torch.mean(video_output[:,:,600:,:], dim=2)
                feat_v_map = self.contra_head_v_map(video_pooled_map)
                feat_v_map = F.normalize(feat_v_map,dim=-1)
                if compute_loss:
                    feat_v = ddp_allgather_with_grads.apply(feat_v)
                    ## det-motion-map
                    feat_v_det = ddp_allgather_with_grads.apply(feat_v_det)
                    feat_v_motion = ddp_allgather_with_grads.apply(feat_v_motion)
                    feat_v_map = ddp_allgather_with_grads.apply(feat_v_map)
            ...
            if compute_loss:
                ...
                ## g_det_motion_map
                contra_loss_tv_g= None 
                contra_loss_tv_det = None 
                contra_loss_tv_motion = None 
                contra_loss_tv_map = None 
                if self.contra_type == 'fine':
                    if 'tv' in contra_task:
                        ...
                        contra_loss_tv_g = self.contrastive_loss(score_matrix_tv).mean()
                        ## det
                        maskA_det = (txt_tokens_contra_det !=0).long().cuda()
                        maskB_det = torch.ones(*feat_v_det.shape[:2]).long().cuda()
                        weightA_det = self.fine_weight_mapper_det['text'](feat_t_det).squeeze(2)
                        weightB_det = self.fine_weight_mapper_det['video'](feat_v_det).squeeze(2)
                        score_matrix_tv_det = self.compute_fine_matrix(feat_t_det, feat_v_det, maskA_det, maskB_det, weightA_det, weightB_det)
                        contra_loss_tv_det = self.contrastive_loss(score_matrix_tv_det).mean()
                        ## motion
                        maskA_motion = (txt_tokens_contra_motion !=0).long().cuda()
                        maskB_motion = torch.ones(*feat_v_motion.shape[:2]).long().cuda()
                        weightA_motion = self.fine_weight_mapper_motion['text'](feat_t_motion).squeeze(2)
                        weightB_motion = self.fine_weight_mapper_motion['video'](feat_v_motion).squeeze(2)
                        score_matrix_tv_motion = self.compute_fine_matrix(feat_t_motion, feat_v_motion, maskA_motion, maskB_motion, weightA_motion, weightB_motion)
                        contra_loss_tv_motion = self.contrastive_loss(score_matrix_tv_motion).mean()
                        ## map
                        maskA_map = (txt_tokens_contra_map !=0).long().cuda()
                        maskB_map = torch.ones(*feat_v_map.shape[:2]).long().cuda()
                        weightA_map = self.fine_weight_mapper_map['text'](feat_t_map).squeeze(2)
                        weightB_map = self.fine_weight_mapper_map['video'](feat_v_map).squeeze(2)
                        score_matrix_tv_map = self.compute_fine_matrix(feat_t_map, feat_v_map, maskA_map, maskB_map, weightA_map, weightB_map)
                        contra_loss_tv_map = self.contrastive_loss(score_matrix_tv_map).mean()
                        ##
                        contra_loss_tv = 0.5*contra_loss_tv_g + 0.17*contra_loss_tv_det + 0.17*contra_loss_tv_motion + 0.17*contra_loss_tv_map
    ...
    def generate_cap(self, batch, task, mode='none'):
        ...
        if 'tv' in task:
            ...
            batch['prompt_input'] = self.get_task_prompt_ltp('describe the video with natural language', commands, batch_size)
            #print(batch['prompt_input'].size())
            if self.beam_size >1:
