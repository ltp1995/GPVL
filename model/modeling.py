...
from .trans_ltp import TransformerEncoder_ltp
...
class VALORModel(VALORPreTrainedModel):
    ...
    def __init__(self, opts):
        ...
        self.config = config
        ##
        num_layers, input_dim, num_heads, hidden_dim=6,256, 8, 512
        self.transformer_ltp = TransformerEncoder_ltp(num_layers, input_dim, num_heads, hidden_dim)
    ...
    def get_task_prompt(self, sentence, batch_size=0,cls_prompt=False, type=None):
        ...
        return task_prompt
    ##
    def get_task_prompt_ltp(self, sentence, commands, batch_size, cls_prompt=False, type=None):
        command_list = []
        if type is None:
            type = self.multimodal_encoder_type
        if not type.startswith('clip'):
            ...
            for command in commands:
                command = self.tokenizer.tokenize(command)
                command = self.tokenizer.convert_tokens_to_ids(command)
                command = [self.bos_token] + command + [self.eos_token]
                command = torch.tensor(command).unsqueeze(0).long().cuda()
                size1, size2 = command.size()
                output = torch.zeros((1, 100), dtype=torch.long).cuda()
                #output = torch.zeros((1, 10), dtype=torch.long).cuda()
                output[:, :size2] = command
                command_list.append(output)
            command_prompt=torch.cat(command_list, dim=0)
            sentence = self.tokenizer.tokenize(sentence)
            sentence = self.tokenizer.convert_tokens_to_ids(sentence)
            task_prompt = [self.bos_token] + sentence + [self.eos_token]
    ...
    def forward_video_encoder(self, video_pixels):   ### b,n,3,h,w
        ...
        ####video_pixels=video_pixels.squeeze(1)  ## (b, 1, 400, dim)
        #print(video_pixels.size())
        #visual_feature = video_pixels[:,:,:300,:]
        #print(video_pixels.size())
        visual_feature = video_pixels.squeeze(1)
        visual_feature_att = self.transformer_ltp(visual_feature).unsqueeze(1)
        ...
        return visual_feature_att  #### B , n , x ,C  n = self.frame_num
    ...
    def forward_shareultimodal_encoder(self, *args,**kwargs):
    ...
    def load_bert_model(self,config):
        ...
        self.multimodal_encoder2 = BertModel(bertconfig)
        bert_weight2 = {k.replace('bert.','').replace('gamma','weight').replace('beta','bias') : v for k,v in bert_weight.items()}

        if self.initial_multimodal:
            missing_keys, unexpected_keys = self.multimodal_encoder.load_state_dict(bert_weight, strict=False)
            missing_keys2, unexpected_keys2 = self.multimodal_encoder2.load_state_dict(bert_weight, strict=False)
    ...
    def construct_text_model(self,config):
        ...
        if self.txt_encoder_type.startswith('bert'):
            if config.share_txt_and_multimodal:
                ...
                self.txt_encoder = self.multimodal_encoder2
                self.txt_dim = self.multimodal_dim
                # from model.bert import BertModel, BertConfig
