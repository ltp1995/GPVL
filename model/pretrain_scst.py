...
import pickle
#with open('/root/data1/ltp/codes/vision-language/VALOR-mine/nuscene_infos/train_val_cmds.pickle', 'rb') as file:
#    train_val_cmds = pickle.load(file)
train_val_prev_trajs=json.load(open('/root/data1/ltp/codes/vision-language/VALOR-mine/VALOR-v1/datasets/nuscene/captions_nuscene_train_val_prevtrajs.json'))
...
class VALOR(VALORModel):
    ...
    def forward_pt(self, batch, task, compute_loss=True):
        ...
        if contra_task != []:
            ...
            if 'v' in ''.join(contra_task):
                ...
                video_pooled = torch.mean(video_output, dim=2)
                #video_pooled = self.mlp(video_pooled)
    ...
    def forward_cap(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            return self.generate_cap_test(batch, task)
    ...
    def generate_cap(self, batch, task, mode='none'):
        ...
        if 'tv' in task:
            ...
            if self.beam_size >1:
                ...
                generated_sequences_t_v, logprobs_t_v = self.decode_greedy(batch, 'cap', mode)
        ...
        return evaluation_dict
    def generate_cap_test(self, batch, task, mode='none'):
    ...
    def decode_beam(self, batch, task):
        ...
        log_probs = []
        selected_words = None
        #seq_mask = torch.ones((batch_size, beam_size, 1), dtype=torch.int64).cuda()
        ...
        for t in range(max_generation_len):
            ...
            #selected_beam=selected_beam.to(torch.int64)
            #selected_words=selected_words.to(torch.int64)
