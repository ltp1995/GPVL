...
class VideoMapper(object):
    ...
    def __getitem__(self, id_):
        ...
        if  self.datatype.startswith('video'):
            #try:
                ### det_map_hs_features_lane (6, 1, 400, 256)
                # det_path = self.video_dir + id_ + '.npy'
                # det_feature = np.load(det_path, allow_pickle=True)
                # det_feature = det_feature[-1]
                # det_feature = torch.tensor(det_feature)
                #det_feature = transforms.ToTensor()(det_feature)
                ### egoquery_mapquery_features (400, 256)
                det_path = self.video_dir + id_ + '.npy'
                det_feature = np.load(det_path, allow_pickle=True)
                det_feature = torch.tensor(det_feature)
                det_feature = det_feature.unsqueeze(0)
                #det_feature = transforms.ToTensor()(det_feature)
                # det_path = self.video_dir + id_ + '.npy'
                # det_feature = np.load(det_path, allow_pickle=True)
                # det_feature = transforms.ToTensor()(det_feature) 
                # det_feature = det_feature.unsqueeze(0)
                ###################################################
                #det_feature = torch.from_numpy(det_feature)
                # video_pixels = []        
                # frame_path = os.path.join(self.video_dir, id_)
                # #print(frame_path)
                # #print(100*'-')
                # #if len(os.listdir(frame_path))>1:
            ...
            #try:
                ...
                img = img.convert('RGB')
                img = transforms.ToTensor()(img)#### convert 1-channel gray image and 4-channel CMYK image to RGB image
                video_pixels = []
                video_pixels.append(img.unsqueeze(0))
                img = torch.cat(video_pixels, dim=0)  ### nX3xHxW
...
class VALORDatasetpre(Dataset):
    def __init__(self, ids_path, txt_mapper_det, txt_mapper_motion, txt_mapper_map, txt_mapper_cap, video_mapper, audio_mapper, training):
        self.txt_mapper_det = txt_mapper_det
        self.txt_mapper_motion = txt_mapper_motion
        self.txt_mapper_map = txt_mapper_map
        self.txt_mapper_cap = txt_mapper_cap
        self.video_mapper = video_mapper
        self.audio_mapper = audio_mapper
        if self.txt_mapper_det is not None:
            self.txt_mapper_det.training = training 
    ...
    def __getitem__(self, i):
        ...
        if self.txt_mapper_det is not None:
            txt_tokens_det = self.txt_mapper_det[id_]
            txt_tokens_motion = self.txt_mapper_motion[id_]
            txt_tokens_map = self.txt_mapper_map[id_]
            txt_tokens_cap = self.txt_mapper_cap[id_]
            if self.training:
                id_txt = id_  
                num_samples = 1 
            else:
                id_txt = [id_] * len(txt_tokens_det)
                num_samples = len(txt_tokens_det)
        if self.video_mapper is not None:
           
            video_pixels = self.video_mapper[id_]
            #print(video_pixels.size)
        ...
        return id_, txt_tokens_det, txt_tokens_motion, txt_tokens_map, txt_tokens_cap, video_pixels, audio_spectrograms, id_txt, num_samples




def valor_collatepre(inputs):
    

    (ids, txt_tokens_det, txt_tokens_motion, txt_tokens_map, txt_tokens_cap, video_pixels, audio_spectrograms, ids_txt, num_samples) = map(list, unzip(inputs))
    ...
    if isinstance (ids_txt[0], list) : ### testing:
        ...
        ids_txt = None 
    ## det
    if txt_tokens_det[0] is not None:
        txt_tokens_det = [ j  for i in txt_tokens_det for j in i]
        txt_tokens_collate_det = {}
        for k in txt_tokens_det[0].keys():  #### bert tokens and clip tokens
            txt_tokens_collate_det[k] = torch.stack([i[k] for i in txt_tokens_det]) 
    else:
        txt_tokens_collate_det = None 
    ## motion
    if txt_tokens_motion[0] is not None:
        txt_tokens_motion = [ j  for i in txt_tokens_motion for j in i]
        txt_tokens_collate_motion = {}
        for k in txt_tokens_motion[0].keys():  #### bert tokens and clip tokens
            txt_tokens_collate_motion[k] = torch.stack([i[k] for i in txt_tokens_motion]) 
    else:
        txt_tokens_collate_motion = None 
    ## map
    if txt_tokens_map[0] is not None:
        txt_tokens_map = [ j  for i in txt_tokens_map for j in i]
        txt_tokens_collate_map = {}
        for k in txt_tokens_map[0].keys():  #### bert tokens and clip tokens
            txt_tokens_collate_map[k] = torch.stack([i[k] for i in txt_tokens_map]) 
    else:
        txt_tokens_collate_map = None 
    ## cap
    if txt_tokens_cap[0] is not None:
        txt_tokens_cap = [ j  for i in txt_tokens_cap for j in i]
        txt_tokens_collate_cap = {}
        for k in txt_tokens_cap[0].keys():  #### bert tokens and clip tokens
            txt_tokens_collate_cap[k] = torch.stack([i[k] for i in txt_tokens_cap]) 
    else:
        txt_tokens_collate_cap = None 
    ...
    batch =   {'ids': ids,
             'txt_tokens_det': txt_tokens_collate_det,
             'txt_tokens_motion': txt_tokens_collate_motion,
             'txt_tokens_map': txt_tokens_collate_map,
             'txt_tokens_cap': txt_tokens_collate_cap,
