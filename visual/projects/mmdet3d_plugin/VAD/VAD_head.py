...
class VADHead(DETRHead):
    ...
    def forward(self,
        ...
        ### save features
        det_motion_map_feature = torch.cat((hs[-1], agent_query, map_query), dim=1).squeeze().detach().cpu().numpy()
        root = './data/nuscenes-ltp/det_motion_map_features_base/val/'
        path1 = root + img_metas[0]['sample_idx']+ '.npy'
        np.save(path1, det_motion_map_feature)
