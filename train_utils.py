...
from data.datapretrain import VALORDatasetpre, valor_collatepre
from data.datacaption import VALORDatasetcap, valor_collatecap
...
def load_from_pretrained_dir(opts):
    ...
    #ckpt_file= 'output/nuscene_base_agentquery_mapquery_prevtrajs/nuscene-caption-lr9e-6-bs68-test6-train6f-bev-9e-6/ckpt/model_step_2618.pt'
        ...
        for dset in d_cfg['datasets']:
            ...
            if 't' in task_short:
                ...
                if d_cfg['task'].startswith('cap'):
                    ...
                    prompt = dset['prompt'] if 'prompt' in dset else None
                    det_path = dset['txt'] + 'captions_pretrain_det.json'
                    motion_path = dset['txt'] + 'captions_pretrain_motion.json'
                    map_path = dset['txt'] + 'captions_pretrain_map.json'
                    cap_path = dset['txt'] + 'captions_finetune_train.json'
                    txt_mapper_det = TxtMapper(det_path, opts, max_txt_len, data_type, prompt = prompt) 
                    txt_mapper_motion = TxtMapper(motion_path, opts, max_txt_len, data_type, prompt = prompt) 
                    txt_mapper_map = TxtMapper(map_path, opts, max_txt_len, data_type, prompt = prompt)
                    txt_mapper_cap = TxtMapper(cap_path, opts, 100, data_type, prompt = prompt)
            ...
            if d_cfg['task'].startswith('cap'):
                dataset = VALORDatasetcap(ids_path, txt_mapper, video_mapper, audio_mapper, training=True)
                collate_fn = valor_collatecap
            else:
                dataset = VALORDatasetpre(ids_path, txt_mapper_det, txt_mapper_motion, txt_mapper_map, txt_mapper_cap, video_mapper, audio_mapper, training=True)
                collate_fn = valor_collatepre
...
def get_args():
    ...
    parser.add_argument('--share_txt_and_multimodal', type=str2bool, default=False)
